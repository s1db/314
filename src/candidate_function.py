...
class CandidateFunction:
    ...
    """
    def __init__(self, node_type: NodeType, children: Optional[List['CandidateFunction']] = None, value: Optional[int] = None):
        self.node_type = node_type
        # Use tuple for immutability and hashing
        self.children: Tuple['CandidateFunction', ...] = tuple(children) if children is not None else ()
    ...
    def size(self) -> int:
        ...
        if self._size is None:
            if self.node_type == NodeType.LITERAL or self.node_type == NodeType.CONSTANT:
                self._size = 1
            else:
    ...
    def depth(self) -> int:
        if self._depth is None:
            if self.node_type == NodeType.LITERAL or self.node_type == NodeType.CONSTANT:
                ...
                self._depth = 1 + max((child.depth for child in self.children), default=0)
        return self._depth
    ...
    def substitute(self, manager: 'FunctionManager', mapping: Dict[int, 'CandidateFunction']) -> 'CandidateFunction':
        """
    ...
    def Not(self, manager: 'FunctionManager') -> 'CandidateFunction':
        ...
        if self.node_type == NodeType.LITERAL:
            ...
            # ~ITE(c, t, e) = ITE(c, ~t, ~e)
            return manager.get_ite(self.children[0], 
...
class FunctionManager:
    ...
    def get_lit(self, value: int) -> CandidateFunction:
        ...
        return self._get_from_cache(key, lambda: CandidateFunction(NodeType.LITERAL, value=value))
    ...
    def get_and(self, children: List[CandidateFunction]) -> CandidateFunction:
        ...
        return self._get_from_cache(key, lambda: CandidateFunction(NodeType.AND, children=list(children_tuple)))
    ...
    def get_or(self, children: List[CandidateFunction]) -> CandidateFunction:
        ...
        return self._get_from_cache(key, lambda: CandidateFunction(NodeType.OR, children=list(children_tuple)))

    def get_ite(self, condition: CandidateFunction, true_branch: CandidateFunction, false_branch: CandidateFunction) -> CandidateFunction:
        # Simplification
        ...
        return self._get_from_cache(key, lambda: CandidateFunction(NodeType.ITE, children=[condition, true_branch, false_branch]))
