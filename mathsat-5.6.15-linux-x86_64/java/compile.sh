...
if [ "$(uname -s)" = "Darwin" ]; then
    ...
    plat=linux
    soext=so
    soname=-Wl,-soname,libmathsatj.so
fi
...
$CC  -pthread -fPIC -g -c -I$JAVA_DIR/include -I$JAVA_DIR/include/$plat -I$MSAT_SRC_DIR -I$GMP_INCLUDE_DIR mathsat_api.c > compile.log 2>&1
$CXX  -pthread -fPIC -g -o libmathsatj.$soext -shared $soname  -I$JAVA_DIR/include -I$JAVA_DIR/include/linux -I$MSAT_SRC_DIR -I$GMP_INCLUDE_DIR -L$MSAT_LIB_DIR -L$GMP_LIB_DIR mathsat_api.o -lmathsat -lgmpxx -lgmp >> compile.log 2>&1

if [ $? -eq 0 ]; then
    if [ "$plat" != "darwin" ]; then
        strip libmathsatj.$soext
    fi
