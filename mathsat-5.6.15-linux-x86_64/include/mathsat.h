...
/**
 * \brief Error checking for types
 *
...
/**
 * \brief returns the type for Booleans in the given env.
 */
msat_type msat_get_bool_type(msat_env env);

/**
 * \brief returns the type for rationals in the given env.
 */
msat_type msat_get_rational_type(msat_env env);

/**
 * \brief returns the type for integers in the given env.
 */
msat_type msat_get_integer_type(msat_env env);

/**
 * \brief returns the type for bit-vectors of the given width.
 */
msat_type msat_get_bv_type(msat_env env, size_t width);

/**
 * \brief returns the type for arrays with indices of \a itp type and
 * elements of \a etp type.
 */
msat_type msat_get_array_type(msat_env env, msat_type itp, msat_type etp);

 /**
 * \brief returns the type for floats with the given exponent and
...
/**
 * \brief returns an enum type in the given env.
 * \param env The environment in which to operate
 * \param name The name of the enum type
 * \param domain_size The number of values of the enum
 * \param domain The possible values of the enum
 * \return an enum type
 */
msat_type msat_get_enum_type(msat_env env, const char *name, size_t domain_size,
                             const char **domain);
...
/**
 * \brief checks whether the given type is an enum
 * 
 * \param env The environment in which to operate
 * \param tp The type to check
 * \param out_domain_size Pointer to a variable where to store the number
 *                        of values in the enum (can be NULL)
 * \param out_domain Pointer to a variable where to store the list of values
 *                   in the enum (can be NULL); if not NULL and the result is 1,
 *                   it must be deallocated by the user with ::msat_free()
 * \return 1 if the type is an enum, 0 otherwise
 */
int msat_is_enum_type(msat_env env, msat_type tp, size_t *out_domain_size,
                      msat_decl **out_domain);


/**
 * \brief checks whether the given type is a simple type
 * 
 * \param env The environment in which to operate
 * \param tp The type to check
 * \param out_name Pointer to a variable where to store the name of the
 *                 simple type (can be NULL); if not NULL and the result is 1,
 *                 it must be deallocated by the user
 *                 with ::msat_free()
 * \return 1 if the type is simple, 0 otherwise
 */
int msat_is_simple_type(msat_env env, msat_type tp, char **out_name);


/**
 * \brief checks whether the given type is a function type
 * 
 * \param env The environment in which to operate
 * \param tp The type to check
 * \param out_num_params Pointer to a variable where to store the number
 *                       of parameters of the function (can be NULL)
 * \param out_param_types Pointer to a variable where to store the list of
 *                        parameter types (can be NULL); if not NULL and the
 *                        result is 1, it must be deallocated by the user
 *                        with ::msat_free()
 * \param out_return_type Pointer to a variable where to store the return type
 *                        (can be NULL)
 * \return 1 if the type is an enum, 0 otherwise
 */
int msat_is_function_type(msat_env env, msat_type tp, size_t *out_num_params,
                          msat_type **out_param_types,
                          msat_type *out_return_type);

/**
 * \brief checks whether two types are the same
...
/**
 * \brief Returns a numeric identifier for the input type
 *
 * The returned value is guaranteed to be unique within the environment in
 * which \a t was defined. Therefore, it can be used to test
 * two type for equality, as well as a hash value.
 *
 * \param t A type. 
...
/**
 * \brief Checks whether \a t is a value for an enum type
 * \param t A term
 * \return nonzero if \a t is an enum value
 */
...
int msat_all_sat_ext(msat_env e, msat_term *important, size_t num_important,
                     msat_truth_value phase,
                     msat_all_sat_model_callback func, void *user_data);
